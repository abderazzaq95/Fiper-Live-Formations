        ...
        {courses.map((course, index) => (
          <article key={course.id} className="group overflow-hidden rounded-[22px] border border-[#dfe7ec] bg-white shadow-[0_12px_35px_rgba(15,42,61,.035)]">
            <div className={`relative h-32 overflow-hidden p-5 ${index === 0 ? "bg-[#082943]" : index === 1 ? "bg-[#102d45]" : "bg-[#17374e]"}`}>
              ...
              <div className="relative flex items-start justify-between"><span className={`rounded-full px-3 py-1.5 text-[8px] font-bold ${statusMap[course.tone]}`}>{course.status}</span><button aria-label="المزيد" className="flex h-8 w-8 items-center justify-center rounded-lg bg-black/15 text-white/70"><MoreHorizontal size={17} /></button></div>
            ...
            <div className="p-5">
              ...
              <div className="mt-5 flex items-center gap-2 border-t border-[#edf2f5] pt-4">
                ...
                <Link href="/" target="_blank" aria-label="معاينة" className="flex h-10 w-10 items-center justify-center rounded-xl border border-[#dfe7ec] text-[#607686]"><Eye size={15} /></Link>
                <button aria-label="نسخ الدورة" className="flex h-10 w-10 items-center justify-center rounded-xl border border-[#dfe7ec] text-[#607686]"><Copy size={15} /></button>
              </div>
            </div>
          </article>
        ))}
