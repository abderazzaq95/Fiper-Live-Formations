...
export default async function AttendancePage() {
  ...
  const stats = [
    ["الحاضرون", String(data.attended), data.rows.length ? `${Math.round((data.attended / data.rows.length) * 100)}% من المسجلين` : "لا توجد بيانات", UserRoundCheck, "bg-[#eaf8f3] text-[#168a65]"],
    ["لم يحضروا", String(data.absent), data.rows.length ? `${Math.round((data.absent / data.rows.length) * 100)}% من المسجلين` : "لا توجد بيانات", UserRoundX, "bg-[#fff0f1] text-[#C32828]"],
  ...
  return (
    <div className="mx-auto max-w-[1450px]">
      ...
      <section className="mt-5 overflow-hidden rounded-[22px] border border-[#dfe7ec] bg-white">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-right"><thead className="bg-[#f8fafb] text-[9px] text-[#7f929f]"><tr><th className="px-6 py-3">المشارك</th><th className="px-4 py-3">الحالة</th><th className="px-4 py-3">وقت الدخول</th><th className="px-4 py-3">وقت الخروج</th><th className="px-4 py-3">مدة الحضور</th><th className="px-4 py-3">المطابقة</th></tr></thead><tbody className="divide-y divide-[#edf2f5]">{data.rows.length ? data.rows.map((person) => { const attended = person.status === "attended"; return <tr key={person.id} className="text-[9px]"><td className="px-6 py-4"><strong className="block text-[10px]">{person.name}</strong><small className="latin mt-1 block text-[8px] text-[#91a2ae]">{person.email}</small></td><td className="px-4 py-4"><span className={`rounded-full px-3 py-1.5 text-[8px] font-bold ${attended ? "bg-[#eaf8f3] text-[#168a65]" : "bg-[#fff0f1] text-[#C32828]"}`}>{attended ? "حضر" : "لم يحضر"}</span></td><td className="latin px-4 py-4 text-[#607686]">{person.joinedAt}</td><td className="latin px-4 py-4 text-[#607686]">{person.leftAt}</td><td className="px-4 py-4"><strong>{attended ? `${person.durationMinutes} دقيقة` : "—"}</strong>{attended && <div className="mt-2 h-1 w-20 overflow-hidden rounded-full bg-[#e8eef2]"><div className="h-full bg-[#1779b5]" style={{ width: `${Math.min(100, (person.durationMinutes / 90) * 100)}%` }} /></div>}</td><td className="px-4 py-4"><span className="flex items-center gap-2 text-[#168a65]"><CalendarCheck2 size={14} /> {methodLabel[person.matchMethod] ?? person.matchMethod}</span></td></tr>; }) : <tr><td colSpan={6} className="px-6 py-14 text-center text-xs text-[#788d9c]">لا توجد سجلات حضور بعد.</td></tr>}</tbody></table></div>
      </section>
    </div>
  );
}