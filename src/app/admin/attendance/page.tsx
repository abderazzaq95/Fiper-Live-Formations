...
export default async function AttendancePage() {
  const [data, courses] = await Promise.all([getDashboardAttendance(), listDashboardCourses()]);
  return <div className="mx-auto max-w-[1450px]">
    ...
    <div className="mt-7"><AttendanceDashboard rows={data.rows} courseOptions={courses.map((course) => course.title)} /></div>
  </div>;
}