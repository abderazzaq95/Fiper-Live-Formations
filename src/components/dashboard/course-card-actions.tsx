...
import { Copy, Eye, Loader2, MoreHorizontal, Power } from "lucide-react";
...
export function CourseCardActions({ courseId, registrationOpen, variant }: CourseCardActionsProps) {
  const router = useRouter();
  const [menuOpen, setMenuOpen] = useState(false);
  const [busy, setBusy] = useState<"copy" | "toggle" | null>(null);
    ...
    return (
      <div className="relative">
        <button type="button" aria-label="خيارات الدورة" aria-expanded={menuOpen} onClick={() => setMenuOpen((open) => !open)} className="flex h-8 w-8 items-center justify-center rounded-lg bg-black/15 text-white/80 transition hover:bg-black/30">
          {busy === "toggle" ? <Loader2 size={15} className="animate-spin" /> : <MoreHorizontal size={17} />}
        </button>
