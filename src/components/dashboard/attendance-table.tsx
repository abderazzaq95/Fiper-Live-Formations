...
export function AttendanceTable({ rows }: { rows: AttendanceRow[] }) {
  ...
  useEffect(() => {
    if (!filterHydrated.current) {
      ...
      setCourse(params.get("course") ?? "all");
  ...
  const exportExcel = () => {
    const data = [[t.participant, t.email, t.course, t.status, `${t.joined} (DE)`, `${t.left} (DE)`, t.duration, t.match], ...filtered.map((person) => [person.name, person.email, person.course, labels[person.status], person.joinedAt, person.leftAt, person.status === "attended" ? `${person.durationMinutes} دقيقة` : "—", matchLabels[person.matchMethod] ?? person.matchMethod])];
    const worksheet = XLSX.utils.aoa_to_sheet(data);
    worksheet["!cols"] = [{ wch: 24 }, { wch: 30 }, { wch: 32 }, { wch: 16 }, { wch: 16 }, { wch: 16 }, { wch: 16 }, { wch: 20 }];
