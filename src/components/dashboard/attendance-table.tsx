...
const t = { email: "البريد", excel: "تصدير Excel", records: "سجلات الحضور", filter: "تصفية مباشرة حسب الدورة والحالة", search: "ابحث عن مشارك...", allCourses: "كل الدورات", allStatuses: "كل الحالات", participant: "المشارك", course: "الدورة", status: "الحالة", joined: "وقت الدخول", left: "وقت الخروج", duration: "مدة الحضور", match: "المطابقة", attended: "حضر", absent: "لم يحضر", pending: "لم تبدأ", sync: "بانتظار المزامنة", empty: "لا توجد نتائج مطابقة." } as const;
const labels: Record<AttendanceRow["status"], string> = { attended: t.attended, absent: t.absent, pending: t.pending };

function DualTime({ germany, turkey }: { germany: string; turkey: string }) {
  return <span className="latin block space-y-1 text-[8px] leading-4"><span className="block"><b className="mr-1 text-[#1574ad]">DE</b>{germany}</span><span className="block"><b className="mr-1 text-[#168a65]">TR</b>{turkey}</span></span>;
}
...
export function AttendanceTable({ rows }: { rows: AttendanceRow[] }) {
  ...
  const filtered = useMemo(() => rows.filter((row) => {
    ...
    return (!query || haystack.includes(query.toLowerCase())) && (course === "all" || row.course === course) && (status === "all" || row.status === status);
  }), [rows, query, course, status]);
  const exportExcel = () => {
    const data = [[t.participant, t.email, t.course, t.status, `${t.joined} (DE)`, `${t.joined} (TR)`, `${t.left} (DE)`, `${t.left} (TR)`, t.duration, t.match], ...filtered.map((person) => [person.name, person.email, person.course, labels[person.status], person.joinedAt, person.joinedAtTurkey, person.leftAt, person.leftAtTurkey, person.status === "attended" ? `${person.durationMinutes} دقيقة` : "—", person.matchMethod === "pending" ? t.sync : person.matchMethod])];
    const worksheet = XLSX.utils.aoa_to_sheet(data);
    worksheet["!cols"] = [{ wch: 24 }, { wch: 30 }, { wch: 32 }, { wch: 16 }, { wch: 16 }, { wch: 16 }, { wch: 16 }, { wch: 16 }, { wch: 16 }, { wch: 20 }];
