...
export function CourseEditor({ courseId, initial, isNew = false }: { courseId: string; initial: CourseEditorData | null; isNew?: boolean }) {
  ...
  const timezoneValue = ["Europe/Berlin", "Europe/Istanbul", "Asia/Qatar"].includes(session?.timezone ?? "") ? session?.timezone ?? "Europe/Berlin" : "Europe/Berlin";
