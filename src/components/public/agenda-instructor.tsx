...
export function AgendaInstructor({ course, agenda }: { course: Course; agenda: { time: string; title: string; text: string }[] }) {
  return (
    <>
          ...
          <div>
            <div className="lg:sticky lg:top-12">
              ...
              <div className="mt-8 hidden rounded-[24px] bg-[#071f34] p-6 text-white lg:block">
                ...
                <p className="mt-2 text-2xl font-bold">{course.duration}</p>
      ...
      <section id="instructor" className="noise-grid relative overflow-hidden bg-[#031a2d] py-24 sm:py-30">
        <div className="mx-auto grid max-w-[1120px] gap-12 px-5 sm:px-8 lg:grid-cols-[.8fr_1.2fr] lg:items-center">
            ...
            <div className="glass relative overflow-hidden rounded-[32px] p-5">
              <div className="relative flex aspect-[.92/1] items-end overflow-hidden rounded-[24px] bg-[radial-gradient(circle_at_50%_25%,#155786_0%,#092b47_42%,#031a2d_78%)]">
                ...
                <div className="relative mx-auto mb-0 flex h-[95%] w-[95%] items-center justify-center rounded-[24px] bg-gradient-to-b from-[#164f76] to-[#061e33] shadow-[0_-20px_80px_rgba(57,143,204,.14)]">
                  {course.instructor.image ? <Image src={course.instructor.image} alt={course.instructor.name} fill sizes="(max-width: 1024px) 80vw, 320px" className="object-cover object-top" /> : <span className="text-6xl font-black tracking-[-0.08em] text-white/90">{course.instructor.initials}</span>}
                </div>
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#031a2d] to-transparent px-6 pb-6 pt-20">
                  ...
                  <p className="mt-1 text-[10px] text-[#7f9cb2]">{course.instructor.role}</p>
                </div>
              </div>
            </div>
          ...
          <div>
            <SectionHeading eyebrow="محاضرك في هذه الدورة" title={course.instructor.name} description={course.instructor.role} />
            <div className="mt-7 flex gap-4">
              <Quote className="mt-1 shrink-0 text-[#C32828]" size={26} />
              <p className="max-w-xl text-sm leading-8 text-[#c3d6e5] sm:text-base">{course.instructor.bio}</p>
            </div>
