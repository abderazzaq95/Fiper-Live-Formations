        ...
        <div className="mx-auto max-w-[1240px] px-5 sm:px-8">
          <div className="flex flex-col justify-between gap-8 lg:flex-row lg:items-end">
            ...
            <div className="rounded-2xl border border-[#dce7ef] bg-white px-5 py-4 text-xs leading-6 text-[#597085] shadow-sm">
              <span className="latin ml-2 text-lg font-bold text-[#C32828]">{duration}</span>
              {landing.about.durationLabel}
            </div>
          </div>
          ...
          <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {outcomes.map((item, index) => {
              ...
              return (
                <article key={`${item.index}-${item.title}`} className="group relative overflow-hidden rounded-[24px] border border-[#dce7ef] bg-white p-6 shadow-[0_16px_45px_rgba(12,43,65,.06)] transition hover:-translate-y-1 hover:border-[#b9cfde]">
          ...
          <div className="relative mx-auto w-full max-w-md">
            <div className="glass rounded-[30px] p-7 sm:p-9">
              <div className="flex items-center justify-between">
                ...
                <span className="rounded-full border border-[#62d5aa]/20 bg-[#62d5aa]/10 px-3 py-1.5 text-[10px] font-bold text-[#62d5aa]">{landing.audience.badgeLabel}</span>
              </div>
              <p className="mt-9 text-xs font-semibold text-[#7696ad]">في نهاية الدورة ستكون قد بنيت</p>
              <p className="mt-3 whitespace-pre-line text-3xl font-extrabold leading-[1.45] tracking-[-0.05em] text-white">{landing.audience.badgeTitle}</p>
              <div className="mt-8 grid grid-cols-3 gap-2 border-t border-white/8 pt-7 text-center">
                {landing.audience.stats.slice(0, 3).map((stat) => <div key={`${stat.value}-${stat.label}`}><span className="latin block text-xl font-bold text-white">{stat.value}</span><span className="text-[9px] text-[#6f8da4]">{stat.label}</span></div>)}
              </div>
            </div>
          </div>

          <div>
            <SectionHeading eyebrow={landing.audience.eyebrow} title={landing.audience.title} description={landing.audience.description} />
            <div className="mt-9 grid gap-3 sm:grid-cols-2">
              {audience.map((item) => <div key={item} className="flex items-start gap-3 rounded-2xl border border-white/8 bg-white/[0.035] p-4 text-xs leading-6 text-[#c9dbea]"><span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#C32828]/12 text-[#D75A5A]"><Check size={14} strokeWidth={3} /></span>{item}</div>)}
            </div>
          </div>
        </div>
      </section>
