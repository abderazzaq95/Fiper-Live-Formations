...
export function Countdown({ target }: { target: string }) {
  ...
  return (
    <div className="grid grid-cols-4 gap-1 sm:gap-2" aria-label="الوقت المتبقي لانطلاق الدورة">
      {items.map((item) => (
        <div key={item.label} className="rounded-2xl border border-white/10 bg-[#031a2d]/75 px-1.5 py-2 text-center sm:px-2 sm:py-2.5">
          <span className="latin block text-lg font-bold tracking-[-0.04em] text-white sm:text-xl">
            {String(item.value).padStart(2, "0")}
          </span>
          <span className="mt-1 block text-[9px] font-medium text-[#7594ab] sm:text-[10px]">{item.label}</span>
        </div>
      ))}
    </div>
  );
